...
const CompetitiveExam = ({competitiveBooks, handleAddToCart, handleAddToWishlist}) => {
   ...
   const books = Array.isArray(competitiveBooks) ? competitiveBooks : [];
...
//   }, [books.length]);
  ...
  return (
    <>
         <section className="competitive-exam  py-8">
            <div className="container mx-auto">
                <div className="flex justify-between mb-4">
                    <div className="flex items-center">
                        {/* <i className="fas fa-book-open text-warning mr-2"></i> */}
                                 <IoBookSharp size={30} color='#fac309' className='mr-2'/>
                        <h4 className="text-xl font-semibold">কম্পিটিটিভ-এক্সাম</h4>
                    </div>
                    <div>
                        <Link href={`/category/কম্পিটিটিভ-এক্সাম`} className='flex items-center'>
                        ...
                        {books.map((book) => (
                            <div key={book.id} className="border border-gray-200 mx-2 bg-white rounded-lg shadow-md w-1/2 md:max-w-[250px] md:min-w-[250px] max-h-[400px] min-h-[400px] flex-shrink-0">
                                <div className="relative">
                                    {book?.percentage !== 0 && (
                                        <span className="absolute top-2 left-2 bg-[#fac309] text-white text-xs font-bold px-1">{book?.percentage}% ছাড়</span>
                                    )}
                                    {book?.flat_discount !== 0 && (
                                        <span className="absolute top-2 left-16 bg-[#fac309] text-white text-xs font-bold px-1">{book?.flat_discount} টাকা ছাড়</span>
                                    )}
                                ...
                                <div className="p-4">
                                    ...
                                    <div className="mt-2">
                                        {book?.discount_price === 0 || book?.discount_price === null ? (
                                            ...
                                            <div className="flex">
                                                <h6 className="text-lg font- medium-gray-800 line-through">{book?.regular_price} টাকা</h6>
